...
## TL;DR
0. Normally, the dimensions in gradient descent is coupled. i.e. you can't optimize one parameter, and then freeze it, and move on to 
...
1. In the eignespace, every gradient step shrinks the error in the i-th eigen-dimension by (1−αλ_i). alpha is the learning rate. lambda is the
eigenvalue in that dimension.

2. With the above insight, one can prove that gradient descent converges the fastest at a learning rate that makes the dimension with the largest eigenvalue
...
3. After working out a bunch of math, the paper shows that in momentum descent, the optimal momentum, beta, should be set at (1 - sqrt(αλ_i)).
...
4. Adam, RMSProp, Momentum all fall into the family of “Linear First Order Methods". There is a mathematical upper bound of how fast these methods 
...
5. In stochastic gradient descent, momentum descent has to balance another tradeoff: there is the deterministic component and a zero-meaned stochastic error term.
